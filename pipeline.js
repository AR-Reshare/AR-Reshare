...
const { AuthenticationHandler, PasswordResetHandler} = require('./classes/securityvalidation');
...
class RequestResetPasswordPipeline extends Pipeline {
    /**
     * Initialises the PasswordResetRequest pipeline
     * @param  {...any} args Arguments to pass to the default pipeline constructor
     */
    constructor(...args) {
        super(...args);
        ...
        this.requestTemplate = RequestTemplateDict['password-reset-request'];
        if (this.requestTemplate === undefined) {
            throw new MissingTemplateError('Unable to find request template for password-reset-request');
        }

        this.responseTemplate = ResponseTemplateDict['password-reset-request'];
        if (this.responseTemplate === undefined) {
            throw new MissingTemplateError('Unable to find response template for password-reset-request');
        }
        // no Push Notifications for now, might want to consider implementing them
    ...
    */
    Execute(req, res) {
        ...
        // validate request
        return this.DataValidate(this.requestTemplate, req.body).then(validated => {
            return PasswordResetHandler.resetRequest(this.db, validated);
        }).then(inputObject => {
            if (inputObject){
                return this.EmailRespond(this.emailTemplate, validated_final['email'], inputObject);
            }
            return false;
        }).catch(err => {
            error_final = err;
